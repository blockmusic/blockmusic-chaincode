...
module.exports = class ControllerAlbum {
    ...
    static async queryAlbumsWithPagination(ctx, pageSize, bookmark) {
        try {
            
            const selector = {assetType: {"$eq": "ALBUM"}}
            const use_index = ["_design/indexTipoAtivoDoc", "indexTipoAtivo"]

            let query = {selector, use_index}
            
            let iterator = await ctx.stub.getQueryResultWithPagination(JSON.stringify(query), parseInt(pageSize), bookmark);
            const albums = await Helper.extractIterator(iterator.iterator, false);
            return{
                data: albums,
